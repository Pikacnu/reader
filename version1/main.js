...
function getPages(isHorizontal, textinput) {
	if (isHorizontal) {
		const maxheight = Math.round(text.offsetHeight / zhheight) / 1.5;
		...
		let data = textinput
			...
			.map((e) => `<span>${e}</span>`)
			.reduce((acc, cur) => {
				if (acc.length + cur.length < maxwidth) {
					return acc + cur;
				}
				return [acc, cur];
			}, [])
			.reduce((acc, cur) => {
				...
				if (acc[acc.length - 1].length < maxheight) {
	...
	const maxheight = Math.round(text.offsetHeight / zhheight) - 3;
	const maxwidth = (Math.round(text.offsetWidth / zhwidth) - 1)/2;
	let temp = textinput;
	console.log(maxheight, maxwidth);
	return temp
		.replaceAll('\n', '₩')
		...
		.reduce((acc, cur) => {
			...
			if (acc.length > 1) {
				...
				let lastheight = last
					.replaceAll(/<span>/g, '')
					.replaceAll(/<\/span>/g, '').length;

				let curheight = cur
					.replaceAll(/<span>/g, '')
					.replaceAll(/<\/span>/g, '').length;

				if (lastheight + curheight < maxheight) {
					acc.push(last + cur);
				} else {
					acc.push(last);
					acc.push(cur);
				...
				cur.reduce((acc, cur) => {
					return (
						...
						cur.reduce(
							(acc, cur) =>
								acc +
								cur.replaceAll(/<span>/g, '').replaceAll(/<\/span>/g, '')
									.length,
							0,
						)
					);
				...
				(prev, page, index) => {
					...
					const currentLines = page.reduce(
						(prev, line, index) => {
							...
							const isReturn =
								prev.count +
									line.replaceAll(/<span>/g, '').replaceAll(/<\/span>/g, '')
										.length >=
								textCount;
							return {
								count:
...
sharebtn.addEventListener('click', () => {
	...
	const url = `${window.location.href.split('?')[0]}?textcount=${textcount}`;
	navigator.clipboard.writeText(url);
	alert('已複製連結');
});

//chapterUpdate(0, false, false);
